...
void            ExtDskInternal_Dsk2ExtDskInternal(DISKIMAGE_UNIT *pUnit, const unsigned char *pDiskImage, const unsigned long DiskImageSize)
{
        ...
        if (pDiskImage!=NULL)
        {
                ...
                if (pExtDsk!=NULL)
                {
                        ...
                        int TrackSize;
                        int     TotalTracks;
                        DSKTRACKHEADER *pTrack = (void *)((uintptr_t)pHeader + sizeof(DSKHEADER));
                        ...
                        for (i=0; i<TotalTracks; i++)
                        {
                                ...
                                pTrack = (void *)((uintptr_t)pTrack + TrackSize);
                        }
                }
        }
}
...
void            ExtDskInternal_ExtDsk2ExtDskInternal(DISKIMAGE_UNIT *pUnit, const unsigned char *pDiskImage, const unsigned long DiskImageSize)
{
        ...
        if (pDiskImage!=NULL)
        {
                ...
                if (pExtDsk!=NULL)
                {
                        ...
                        int     TotalTracks;
                        EXTDSKTRACKHEADER *pTrack = (void *)((uintptr_t)pHeader + sizeof(EXTDSKHEADER));
                        ...
                        for (i=0; i<TotalTracks; i++)
                        {
                                ...
                                if (pHeader->TrackSizeTable[i]!=0)
                                {
                                        ...
                                        pTrack = (void *)((uintptr_t)pTrack + ((pHeader->TrackSizeTable[i] & 0x0ff)<<8));
                                }
                        }
                }
        }
