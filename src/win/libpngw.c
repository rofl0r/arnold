...
#include "..\..\..\png\png.h"
#include "zlib.h"
#include "..\..\..\unzip\ioapi.h" /* zlib_filefunc_def_s */
#include "libpngw.h"
...
const char* const PUB_png_fileext = "png";

static const char* MOD_text_key[] =
{
   "Title",
   "Author",
   "Description",
...
C_ASSERT(_countof(MOD_text_key) == ENUM_pngtext_enumcount);
...
HBITMAP LoadPngZ(voidpf stream, struct zlib_filefunc_def_s* file_api_ptr, char* pText[/*ENUM_pngtext_enumcount*/])
{
   ...
   if (bOK)
   {
      ...
      if (pText != NULL) for (i = 0; i < info_ptr->num_text; i++)
      {
         ...
         {
            if ( (pText[j] != NULL) && (0 == stricmp(info_ptr->text[i].key, MOD_text_key[j])))
            {
   ...
   if (pText != NULL)
	{
      ...
      {
		   text_ptr[i].key  = (char*)MOD_text_key[ENUM_pngtext_title];
      ...
      {
		   text_ptr[i].key  = (char*)MOD_text_key[ENUM_pngtext_author];
      ...
      {
		   text_ptr[i].key  = (char*)MOD_text_key[ENUM_pngtext_desc];
      ...
      {
		   text_ptr[i].key  = (char*)MOD_text_key[ENUM_pngtext_software];
